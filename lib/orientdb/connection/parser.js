...
var serializeFieldValue = function(value) {
    ...
    if (_.isString(value)) {
        ...
        var isSet = value['@type'] == 'Set';
        result = result.concat(isSet?"<":"[");
...
var serializeDocument = function(document, isMap) {
    ...
    for (var field in document) {
        ...
        if (field === "@class") {
            ...
            // workaround for in/out fields converted to Lists
            if ((field == 'in' || field == 'out') && _.isArray(value) && value['@type'] != 'Set') {
                console.log('Forced conversion of field %s from List to Set', field);
                value['@type'] = 'Set';
            }
