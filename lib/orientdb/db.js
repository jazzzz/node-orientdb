...
Db.prototype.getClusterByName = function(clusterName) {
    ...
    return _.find(this.clusters, function(cluster) {
        return cluster.name === lowerCaseClusterName;
    });
};
...
Db.prototype.getDataSegmentById = function(dataSegmentId) {
    return _.find(this.configuration.dataSegments, function(dataSegment) {
        return dataSegment.dataId === dataSegmentId;
    });
};
...
Db.prototype.getClusterByClass = function(className) {
    ...
    if (clazz) {
        var cluster = this.getClusterById(clazz.defaultClusterId);
        if (cluster)
            return cluster;
...
Db.prototype.getClassByName = function(className) {
    return _.find(this.classes, function(clazz) {
        return (clazz.name === className || clazz.shortName === className);
    });
};
...
function loadDbSchema(self, callback) {
    ...
    async.waterfall([
        self.loadRecord.bind(self, "#0:0"),
        function(result, cache, cb) {
            self.configuration = parser.parseConfiguration(result.data.toString());
            self.loadRecord(self.configuration.schemaRecordId, cb);
        },
        function(result, cache, cb) {
...
Db.prototype.create = function(callback) {
    ...
    return this.server.send(OperationTypes.DB_CREATE, data, callback);
};
...
Db.prototype.close = function(callback) {
    ...
    return this.server.send(OperationTypes.DB_CLOSE, callback);
};
...
Db.prototype.exist = function(callback) {
    ...
    return this.server.send(OperationTypes.DB_EXIST, data, function(err, result) {
...
Db.prototype.reload = function(callback) {
    ...
    return this.server.send(OperationTypes.DB_RELOAD, reloadCallback);
};
...
Db.prototype.drop = function(callback) {
    ...
    return this.server.send(OperationTypes.DB_DELETE, data, function(err, result) {
...
Db.prototype.size = function(callback) {
    ...
    return this.server.send(OperationTypes.DB_SIZE, function(err, result) {
...
Db.prototype.countRecords = function(callback) {
    ...
    return this.server.send(OperationTypes.DB_COUNTRECORDS, function(err, result) {
...
Db.prototype.select = function(command, options, callback) {
    if (_.isFunction(options)) {
        callback = options;
        options = {};
    }
    options = options || {};
    options.mode = 'selectSync';
    return this.command(command, options, callback);
};

Db.prototype.selectAsync = function(command, options, callback) {
    if (_.isFunction(options)) {
        callback = options;
        options = {};
    }
    options = options || {};
    options.mode = 'selectAsync';
    return this.command(command, options, callback);
};
...
Db.prototype.command = function(command, options, callback) {
    ...
    data.mode = _.isString(options.mode) ? options.mode : "";
    data.fetchPlan = _.isString(options.fetchPlan) ? options.fetchPlan : "";
    data.non_text_limit = _.isNumber(options.non_text_limit) ? options.non_text_limit : -1;
    var fetchORIDs = _.isBoolean(options.fetchORIDs) ? options.fetchORIDs : true;

    var self = this;

    return self.server.send(OperationTypes.COMMAND, data, function(err, result) {
        if (err) { return callback(err); }

        if (!fetchORIDs) { return callback(null, result.records, result.cache); }

        toRidsFromORIDsOfDocuments(self, result.records, function(err, res) {
            callback(err, res, result.cache);
        });
    });
};
...
Db.prototype.addDataCluster = function(clusterOptions, callback) {
    ...
    return this.server.send(OperationTypes.DATACLUSTER_ADD, data, function(err, result) {
...
Db.prototype.removeDataCluster = function(clusterNumber, callback) {
    ...
    return this.server.send(OperationTypes.DATACLUSTER_REMOVE, data, callback);
};
...
Db.prototype.countDataClusters = function(clustersId, callback) {
    ...
    return this.server.send(OperationTypes.DATACLUSTER_COUNT, data, callback);
};
...
Db.prototype.rangeDataClusters = function(clusterId, callback) {
    ...
    return this.server.send(OperationTypes.DATACLUSTER_DATARANGE, data, callback);
};
...
Db.prototype.createRecord = function(recordData, callback) {
    ...
    if (!/^[bdf]$/.test(recordData.type)) {
        return callback("Invalid record type. Valid values are: b, d, and f");
    }

    var recordType = recordData.type.charCodeAt(0);
    ...
    return this.server.send(OperationTypes.RECORD_CREATE, data, callback);
};
...
function recursivelyAccumulateORIDsTuples(document, tuples) {
    for (var field in document) {
        ...
        if (_.isObject(fieldValue) && null != fieldValue["@class"]) {
...
function toRidsFromORIDsOfDocument(self, document, callback) {
    if (null == document || document["@type"] !== "d") {
...
Db.prototype.loadRecord = function(rid, options, callback) {
    ...
    return self.server.send(OperationTypes.RECORD_LOAD, data, function(err, result) {
        if (err) { return callback(err); }

        if (!result) { return callback("A null or undefined record was returned."); }

        var record = result.record;
        if (!record) { return callback("Record " + rid + " does not exist"); }
        record["@rid"] = rid;

        if (!fetchORIDs) { return callback(null, record, result.cache); }

        toRidsFromORIDsOfDocument(self, record, function(err, record) {
            callback(err, record, result.cache);
        });
    });
};
...
Db.prototype.loadRecords = function(rids, options, callback) {
    ...
    for (var i = 0; i < ridsLength; i++) {
        this.loadRecord(rids[i], options, function(err, record, recordCache) {
            if (err) { return callback(err); }
            records.push(record);
            for (var rid in recordCache) {
                cache[rid] = recordCache[rid];
            }

            if (records.length === ridsLength) {
                return callback(null, records, cache);
            }
        });
    }
};
...
Db.prototype.updateRecord = function(recordData, callback) {
    ...
    return this.server.send(OperationTypes.RECORD_UPDATE, data, callback);
};
...
Db.prototype.delete = function(document, callback) {
    ...
    return this.deleteRecord(recordData, callback);
};
...
Db.prototype.deleteRecord = function(recordData, callback) {
    ...
    return this.server.send(OperationTypes.RECORD_DELETE, data, callback);
};
...
Db.prototype.save = function(document, callback) {
    ...
    if (null != document["@rid"]) {
        var rid = parser.parseRid(document["@rid"]);
        if (null == rid) {
            return callback("The RID must be a string having the format: #<cluster id>:<cluster position>");
        }
        parser.mergeHashes(recordData, rid);
    }
    ...
    var saveDocument = function(recordData) {
        ...
        if (null != recordData.clusterPosition) {
...
Db.prototype.countRecordsInCluster = function(clusterName, callback) {
    ...
    return this.server.send(OperationTypes.COUNT, clusterName, function(err, result) {
