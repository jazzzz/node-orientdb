...
function readPayloadRecord() {
    this.readRecord(function(record) {
        var result = this.result;
        this.result.content = record;
        this.result.records.push(this.processResult(record));
    });
}
...
function readPayloadString() {
    this.readString(function(str) {
        ...
        var content = str;
        content["@type"] = "f";
        this.result.records.push(content);
    });
}

function processCollection() {
    var content = this.result.content;
    var records = [];
    if (content)
        for (var i in content) {
            records.push(this.processResult(content[i]));
        }
