...
command.prototype.readRecord = function(callback) {
    ...
    this.readShort(function(clazz) {
        if (clazz === -2) {
            ...
            throw new Error("Unknown record class id: " + clazz);
        }
    });
};
...
command.prototype.read = function(buf) {
    ...
    while (this.reader != null && totalBytesRead + this.reader.bytes <= localBuffer.length) {
...
command.prototype.processRecordContent = function(record, type, content) {
    ...
    switch (type) {
        ...
        case "d":
            if (parser.isNullOrUndefined(content)) {
                record = {};
            } else {
        ...
        default:
            throw new Error("Invalid or not supported record type: " + type);
    }
};
